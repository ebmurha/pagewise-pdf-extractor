...
Before the API is stable, use `0.x.y` versions. For `rag-engine`, prefer a release tag such as `v0.1.2` over a raw commit SHA once the first integration build is accepted.
